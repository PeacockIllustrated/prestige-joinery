...
const TaskGroup: React.FC<{title: string, tasks: Task[]}> = ({ title, tasks }) => (
    <div>
        <h3 className="text-lg font-bold text-prestige-charcoal mb-3 pb-2 border-b border-prestige-gray">{title}</h3>
        {tasks.length > 0 ? (
            <div className="space-y-2">
                {tasks.map(task => <ClientPortalTaskItem key={task.id} task={task} />)}
            </div>
        ) : (
            <p className="text-sm text-prestige-text italic">No tasks in this category.</p>
        )}
    </div>
);
