...
const ClientPortalTaskItem: React.FC<ClientPortalTaskItemProps> = ({ task }) => {
    ...
    return (
        <div className="flex items-start space-x-4 p-3 bg-prestige-light-gray/60 rounded-lg">
            <div className="mt-1">{getStatusIcon()}</div>
            <div className="flex-grow">
                <p className="font-semibold text-prestige-charcoal">{task.title}</p>
                 {task.description && <p className="text-sm text-prestige-text mt-1">{task.description}</p>}
                <div className="flex items-center space-x-4 mt-2 text-xs text-prestige-text">
