...
const ClientProjectDetailView: React.FC<ClientProjectDetailViewProps> = ({ project, tasks, documents, onBack }) => {
    ...
    const TabButton: React.FC<{
        ...
        <button
            ...
            className={`flex items-center space-x-2 px-4 py-2 rounded-lg font-semibold transition-colors ${
                activeTab === tabName
                    ? 'bg-prestige-teal text-prestige-charcoal'
                    : 'text-prestige-text hover:bg-prestige-gray'
            }`}
        >
            {icon}
            <span>{label}</span>
    ...
    return (
        <div className="space-y-6">
            <div>
                <button onClick={onBack} className="flex items-center text-sm font-semibold text-prestige-text hover:text-prestige-charcoal mb-4">
                    <ChevronLeftIcon className="w-5 h-5 mr-1" />
                    Back to All Projects
                </button>
                <h1 className="text-3xl font-bold text-prestige-charcoal">{project.name}</h1>
                <p className="mt-1 text-prestige-text">{project.address}</p>
            </div>

            <div className="bg-white rounded-xl shadow-md border border-prestige-gray/50">
                <div className="p-3 border-b border-prestige-gray flex items-center space-x-2">
