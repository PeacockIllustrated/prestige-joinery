...
const AdminGuide: React.FC = () => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-prestige-gray/50">
      <div className="p-6 border-b border-prestige-gray flex items-center space-x-3">
        <BookOpenIcon className="w-6 h-6 text-prestige-teal"/>
        <h2 className="text-xl font-bold text-prestige-charcoal">Admin Guide</h2>
      </div>
      <div className="divide-y divide-prestige-gray">
        {fullGuideContent.map((item, index) => (
            <div key={item.title} className="border-b border-prestige-gray last:border-b-0">
                <button
                    onClick={() => handleToggle(index)}
                    className="flex justify-between items-center w-full p-4 text-left font-semibold text-prestige-charcoal hover:bg-prestige-light-gray/50 transition-colors"
                >
                ...
                <div className={`grid transition-all duration-300 ease-in-out ${openIndex === index ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden">
                         <div className="p-4 pt-0 text-sm text-prestige-text" dangerouslySetInnerHTML={{ __html: item.content }} />
                    </div>
                </div>
            </div>
        ))}
