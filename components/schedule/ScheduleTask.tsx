...
const ScheduleTask: React.FC<ScheduleTaskProps> = ({ task, onClick, compact }) => {
    if (compact) {
        return (
            ...
            >
                <p className="text-xs font-semibold text-prestige-charcoal truncate" style={{ color: task.project.color }}>{task.title}</p>
            </div>
        )
    }
    ...
    return (
        <div
            onClick={onClick}
            className="p-1.5 rounded-md cursor-pointer border-l-4 hover:bg-prestige-gray transition-colors"
            style={{ borderColor: task.project.color, backgroundColor: `${task.project.color}1A` }} // 1A for 10% opacity
        >
            <p className="text-xs font-bold text-prestige-charcoal truncate">{task.title}</p>
            <p className="text-xs text-prestige-text truncate" style={{ color: task.project.color }}>{task.project.name}</p>
        </div>
    );
};
