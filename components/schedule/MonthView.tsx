    ...
    return (
        <div className="overflow-x-auto h-full">
            <div className="grid grid-cols-7 min-w-[700px]">
                {dayHeaders.map(day => (
                    <div key={day} className="text-center font-bold text-sm text-prestige-text py-2 border-b border-r border-prestige-gray">
                        {day}
                    </div>
                ))}
                ...
                {days.map((day, index) => {
                    ...
                    return (
                        <div 
                            key={index} 
                            className={`border-b border-r border-prestige-gray p-2 flex flex-col h-28 ${!isCurrentMonth ? 'bg-prestige-light-gray/50' : 'bg-white'}`}
                        >
                            <div className="flex justify-end">
                                <span className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-full ${isToday(day) ? 'bg-prestige-teal text-prestige-charcoal' : 'text-prestige-text'}`}>
                                    {day.getDate()}
                                </span>
                            </div>
                            <div className="flex-grow space-y-1 mt-1 overflow-y-auto">
                                ...
                                {tasksForDay.length > 3 && (
                                    <div className="text-xs text-prestige-text font-semibold px-1">+ {tasksForDay.length - 3} more</div>
                                )}
                            </div>
                        </div>
                    );
