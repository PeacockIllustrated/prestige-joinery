...
const ProjectTaskGroup: React.FC<ProjectTaskGroupProps> = ({ project, onSelectTask }) => {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-prestige-gray/50 p-6">
      <div className="mb-4">
        <h2 className="text-xl font-bold text-prestige-charcoal truncate">{project.name}</h2>
        <div className="flex items-center mt-2">
            <ProgressBar progress={project.progress} />
            <span className="ml-4 text-sm font-bold text-prestige-charcoal w-12 text-right">{project.progress}%</span>
        </div>
      </div>
